...
func (s *sandbox) Exec(ctx context.Context, cmd executor.Cmd) (int32, error) {
	...
	// holds the work runs on. Cancellation is the ONLY way a job is stopped in
	// production (internal/engine cancels the context; nothing calls Signal),
	// so this is the path that has to reach the whole tree.
