...
- [ ] Run `make conformance ENGINE="dhole-engine --executor vm"` — expect PASS. NOT DONE. The engine conformance suite needs a plane, a bus and an object store beside a KVM host, which is a different harness from the executor contract; the executor half is what this task proved.
